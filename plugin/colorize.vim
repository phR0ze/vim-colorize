...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
function! s:ColorizeDevicons(colormap)
  let colors = keys(a:colormap)
  augroup colorizeDevicions
    autocmd!

    " Setup event handlers to highlight icons according to type
    for color in colors

      " Set up highlight group e.g. 'devicons_green'
      exec 'autocmd FileType nerdtree,startify highlight devicons_'.color.
        \ ' guifg=#'.a:colormap[color][0].' ctermfg='.s:rgb(a:colormap[color][0])
      ...
      exec 'autocmd FileType nerdtree,startify syntax match devicons_'.color.
        \ ' /\v'.join(a:colormap[color][1], '|').'/ containedin=ALL'
    endfor
  augroup END
endfunction
